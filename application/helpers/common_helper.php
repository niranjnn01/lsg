<?php if (! defined ( 'BASEPATH' ))	exit ( 'No direct script access allowed' );
	...
	function load_files ($sType, $bSpecifics=false, $aSpecificFiles=array()) {
		...
		*/
		...
		$aControllerSpecificCommonFiles = $CI->mcontents['load_common_' . $sType];
		$aCommonAssetFiles = array_merge($aCommonAssetFiles, $aControllerSpecificCommonFiles);
		...
		if (file_exists( $sParsedFileFullPath ) ){ 
			...
			if( ( ENVIRONMENT_ == 'local' ) || ( $regenerate == true ) ) {
	 ...
	 * will check for user login status, and display the login or logout buttons respectively
	 */
	function login_logout_old_2_11_2012(){
		...
		if($aViewData['iCurrentUser'] = $CI->authentication->is_user_logged_in(false)){
			$aViewData['bLoggedIn']	= true;
			$aViewData['sLogoutUrl'] = c('base_url').'logout';
		} else {
			$aViewData['bLoggedIn'] = false;
			$aViewData['sLoginUrl'] = $CI->facebook->getLoginUrl(array(
		...
		if( $aViewData['bLoggedIn'] ) {
			$CI->mcontents['load_js'][] = 'login_logout.js';
		}
		
		echo $CI->load->view('login_logout', $aViewData, true);
	}
	...
	function hasAccess( $aAllowedUserTypes, $sRedirectTo = '' ) {
		...
		if( $CI->session->userdata('USER_TYPE') == $aUserTypes['admin'] ) {
			...
			foreach( $aUserTypes AS $sKey => $iValue ) {
