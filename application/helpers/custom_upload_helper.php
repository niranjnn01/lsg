 ...
 * @return unknown
 */
function uploadFile($sFileType, $sSection, $sFieldName, $aConfig=array(), $sFileName='') {
	...
	if( $CI->config->item($sSection . '_upload_settings') ) {
		$aConfig = array_merge( $aConfig, $CI->config->item( $sSection . '_upload_settings' ) );
 ...
 *
 */
function requireUploaderFiles( $sSection, $aUploadSettings=array() ) {
	...
	$CI->mcontents['load_js']['regenerate'] = true;
